...
class FileTypeFinder(using Materializer, ExecutionContext) {
  def find(source: => Source[ByteString, Future[IOResult]]):Future[FileType] = {
    ...
    val stream: Source[FileType, Future[IOResult]] = source
      .map { b =>
        ...
        if(str.contains("{") || str.contains("[")) then FileState.Identified(str) else FileState.Detecting(str)
      }
      .takeWhile(x => x match {
      ...
      .map { (x:FileState) =>
        ...
        if(objectStreamIdentifierIndex != -1 && arrayIdentifierIndex != -1) {
          if (objectStreamIdentifierIndex.min(arrayIdentifierIndex) == objectStreamIdentifierIndex)
          then FileType.ObjectStream
          else FileType.Array
        }
        else if(objectStreamIdentifierIndex != -1) FileType.ObjectStream
        else if(arrayIdentifierIndex != -1) FileType.Array
        else throw UnknownFileTypeException()
      }

    stream.toMat(Sink.last)(Keep.right).run()
      .recover{
        case x:NoSuchElementException => throw UnknownFileTypeException()
      }
  }
}
object FileTypeFinder {
  class UnknownFileTypeException extends RuntimeException("Could not identify file type")
}
